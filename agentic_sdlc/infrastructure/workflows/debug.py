...
def run_debug(issue: str, search_kb: bool = True):
    ...
    if search_kb:
        ...
        print('   python bin/kb_cli.py search "[error message]"')
        print('   python tools/neo4j/learning_engine.py --similar-errors "[ErrorType]"')
        print()
    
    print_warning("After fixing, record the pattern:")
    print('   python tools/neo4j/learning_engine.py --record-error "ErrorType" "description" --resolution "fix"')
    print()
