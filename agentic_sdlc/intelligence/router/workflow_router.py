...
class WorkflowRouter:
    ...
    def analyze_complexity(self, request: str) -> TaskComplexity:
        ...
        scope_keywords = {
            ...
            "large": ["system", "architecture", "full", "complete", "entire", "all", "redesign"]
        }
        factors["scope"] = 0
        ...
        complexity_score = max(1, min(10, raw_score + 1))  # Normalize to 1-10
        
        # Determine execution mode
        if factors["multi_role"] >= 2:
            if factors["research_needed"] > 0:
                execution_mode = ExecutionMode.HEAVY_SWARM  # Multiple experts + synthesis
            else:
                execution_mode = ExecutionMode.PARALLEL  # Parallel execution
        elif complexity_score >= 7:
            execution_mode = ExecutionMode.SEQUENTIAL  # Fallback to standard for high complexity
        else:
            execution_mode = ExecutionMode.SEQUENTIAL  # Standard pipeline
