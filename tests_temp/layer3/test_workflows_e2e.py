...
def test_health_check_dry_run():
    script = ROOT_DIR / "agentic_sdlc" / "infrastructure" / "validation" / "health-check.py"
    if not script.exists():
        pytest.fail(f"health-check.py not found at {script}")
...
def test_cycle_workflow_exists():
    script = ROOT_DIR / "agentic_sdlc" / "infrastructure" / "workflows" / "cycle.py"
