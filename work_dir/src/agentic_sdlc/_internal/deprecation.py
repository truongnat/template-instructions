...
def create_module_deprecation_handler(
    ...
    def __getattr__(name: str) -> Any:
        ...
        # handled or if old and new paths are the same.
        # We allow same path ONLY if lenient is True (to handle missing members).
        ...
        # If we are a real module being made lenient (old == new),
        # we don't need to import anything, just return the mock if we haven't found it.
        if old_module_path == new_module_path:
            if lenient:
                from unittest.mock import MagicMock
                return MagicMock(name=f"{old_module_path}.{name}")
