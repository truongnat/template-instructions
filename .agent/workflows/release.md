...
python agentic_sdlc/infrastructure/release/release.py changelog --sprint 6

# Bump version only
python agentic_sdlc/infrastructure/release/release.py version --auto

# Full release cycle
python agentic_sdlc/infrastructure/release/release.py release --commit --tag --push --publish
```
...
### 1. Preview Changes
```bash
python agentic_sdlc/infrastructure/release/release.py preview --sprint 6
```
Shows:
- Last git tag
- Current version
...
```bash
python agentic_sdlc/infrastructure/release/release.py changelog --sprint 6 --dry-run
```
...
### 4. Full Release
```bash
python agentic_sdlc/infrastructure/release/release.py release --sprint 6 --commit --tag --push --publish
```
Executes:
1. Parse commits
2. Detect bump type
...
- **Update Methods:**
  ...
  - **Source Update:** `python agentic_sdlc/infrastructure/update/updater.py`
...
- **Publish to NPM:** To publish to NPM, ensure you are logged in using `npm login` then run the release command with the --publish flag.
  ```bash
  python agentic_sdlc/infrastructure/release/release.py release --publish
  ```
